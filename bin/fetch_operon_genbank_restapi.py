...
for key in n_key_list:
    ...
    ftp_targ_file = [s for s in ftp_file_list if 'feature_table' in s]
    if len(ftp_targ_file) == 0:
          skip_folder.append(key)
          print('Feature table file not found in the ftp server, skipping: ' + key)
          ftp_ls.close()
          continue;
    ...
    loc_file = open(output_path + '/' + sp_name + '/' + ftp_cnt_file[0], 'wb')
    #loc_file = open('tmp.txt.gz', 'wb')

    # perform a ftp get on the file which is to be downloaded in chunks of 1024 bytes
    print("Downloading " + str(n_key_list.index(key)+k_offset) + ' ' + ftp_cnt_file[0] + ' from the ncbi ftp server...')
    ftp_ls.retrbinary('RETR ' + ftp_cnt_file[0], loc_file.write, 1024)
...
for key in n_key_list:
    ...
    txt_file = gz_file[0:-3]
    
    print ('Gunzip ' + str(n_key_list.index(key)) + ':' + gz_file)
